...
use fdd\casbin\model\Rule;
use Casbin\Model\Model;
use Casbin\Persist\Adapter as BaseAdapter;
...
class Adapter implements BaseAdapter
{
    ...
    public function savePolicyLine($ptype, array $rule)
    {
        ...
        $this->model->cache('fddcasbin')->save($col);
    }
    ...
    {
        $rows = $this->model->cache('fddcasbin')->select()->toArray();
        foreach ($rows as $row) {
    ...
    public function removePolicy(string $sec, string $ptype, array $rule): void
    {
        ...
        foreach ($instance->select() as $model) {
            if ($model->cache('fddcasbin')->delete()) {
                ++$count;
            }
        }
    }
